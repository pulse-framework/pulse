...
class Pulse {
  constructor({
    ...
    this._collections.base = new Base(
      this._global,
      { data, indexes, actions, filters, routes, groups },
      request
    );
    this._global.collectionNamespace.push('base');

    this._collections.request = new Request(this._global, request);
    this._global.collectionNamespace.push('request');
  ...
  install(Vue) {
    ...
    Vue.mixin({
      beforeCreate() {
        ...
        this.$auth = self._collections.request;
        // collections
