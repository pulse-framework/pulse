...
class Pulse {
  ...
  assignStorage(storage) {
    const storageAPI = {};
    //set
    if (storage.set) storageAPI.set = storage.set;
    if (storage.setItem) storageAPI.set = storage.setItem;
    //get
    if (storage.get) storageAPI.get = storage.get;
    if (storage.getItem) storageAPI.get = storage.getItem;
    //remove
    if (storage.remove) storageAPI.remove = storage.remove;
    if (storage.removeItem) storageAPI.remove = storage.removeItem;
    //clear
    if (storage.clear) storageAPI.clear = storage.clear;
  ...
  processRegenQueue() {
    ...
    if (this._global.regenQueue.length > 0)
      setTimeout(() => {
        this.processRegenQueue();
      }, 1000);
