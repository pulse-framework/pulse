...
module.exports = class Request extends Collection {
  constructor(
    ...
    this._applyHeaders.credentials = data.credentials;
    this._applyHeaders.mode = data.mode;
    if (headers)
      Object.keys(headers).forEach(header => {
        this._applyHeaders.headers[header] = headers[header];
      });
  ...
  send(url, method, body, headers) {
    return new Promise((resolve, reject) => {
      ...
      let allHeaders = this._applyHeaders;
      if (headers)
        Object.keys(headers).forEach(header => {
