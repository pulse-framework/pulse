...
module.exports = class Collection {
  constructor(
    ...
    this._actionRefrence = {
      ...
      findGroup: this.findGroup.bind(this)
    };
    ...
    this._filterIndexLinks = Object.create(null);
    this._mutableData = [];
  ...
  initProxy(obj) {
    return new Proxy(obj || {}, {
      ...
      get: (target, key, value) => {
        ...
        } else if (this._global.initComplete && typeof key === 'string') {
  ...
  deliverUpdate(type, data, name) {
    ...
    // console.log('UPDATE DELIVERED', name, data);
  ...
  updateSubscribers(key, data) {
    if (this.watchers.hasOwnProperty(key))
      this.watchers[key](this._global.dataRef);
  ...
  processDataItem(data, index) {
    ...
    this.findFilterIndexesToRegen(data[this._primaryKey], index);
  ...
  findFilterIndexesToRegen(primaryKey, indexes) {
    ...
    const checkIndexLinks = index => {
      Object.keys(this._filterIndexLinks).forEach(filterName => {
        if (this._filterIndexLinks[filterName].includes(index)) {
          // push filter directly into regen queue
          if (!this._filtersToForceRegen.includes(filterName))
            this._filtersToForceRegen.push(filterName);
        }
    ...
    if (Array.isArray(indexes)) {
      for (let index of indexes) checkIndexLinks(index);
    } else {
      let index = indexes;
      checkIndexLinks(index);
    }
  }
  ...
  findGroup(id) {
    if (this._executing) this._filterIndexLinks[this._executing] = [id];

    if (this._indexes[id]) return this.buildDataFromIndex(id);
    else return [];
  }
