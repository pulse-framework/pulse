...
module.exports = class Collection {
  ...
  initData(data) {
    Object.keys(data).forEach(key => {
      ...
      this.initDeepReactivity(data, key, data[key]);
    });
  ...
  initProxy(obj = {}, rootProperty = false) {
    ...
    let objectWithCustomPrototype = Object.create({ rootProperty });
    ...
    return new Proxy(objectWithCustomPrototype, {
      set: (target, key, value) => {
        ...
        if (!this._global.initComplete) {
          ...
          // Allow Pulse to internally modify the proxy without intercepting
          ...
          this.initDeepReactivity(target, key, value);
  ...
  initDeepReactivity(target, key, value) {
    let pulse = this;
    let nextToAnalyse = [];
    let rootProperty;
    ...
    function convertToProxy(_target, rootProperty) {
      pulse._allowInternalChange = true;
      _target[key] = pulse.initProxy(value, rootProperty);
      pulse._allowInternalChange = false;
    }
    ...
    if (isObject(value)) {
      ...
      for (let property of Object.keys(value)) {
        if (isObject(value[property])) {
          nextToAnalyse.push(value[property]);
        }
      }

      const loop = () => {
        let next = nextToAnalyse.splice(0);
        nextToAnalyse = [];
        for (let object of next) {
          for (let property of Object.keys(object)) {
            if (isObject(object[property])) {
              nextToAnalyse.push(object[property]);
              convertToProxy(object[property], rootProperty);
            }
          }
        }
