...
module.exports = class Collection {
  ...
  initStorage() {
    if (
      ['sessionStorage', 'localStorage'].includes(this._global.storage.type)
    ) {
      // GET METHOD
      ...
      // GET METHOD
      this._storage.get = key =>
        Promise.resolve(this._global.storage.get(key)).then(res => res);

      // SET METHOD
      this._storage.set = (key, value) => this._global.storage.set(key, value);
    }
  ...
  async initPersist(persist = []) {
    for (let property of persist) {
      ...
      let data;
      if (typeof this._storage.get.then == 'function')
        data = await this._storage.get(storageKey);
      else data = this._storage.get(storageKey);
  ...
  remove(data, group) {
    ...
    if (typeof data[0] == 'object' && data[0].hasOwnProperty(this._primaryKey)) {
      // we're dealing with full data, so we need to validate and map to ids
      data = data.map(item => item[this._primaryKey]);
    } else if (Number.isInteger(data[0])) {
      return assert(`Unable to remove data: '${data[0]}'`);
    }
