...
export default class Collection {
  ...
  executeAndAnalyseFilter(filter) {
    ...
    for (let dependency of found) {
      ...
      depGraph[this._name][filter].dependencyNames.push(dependency.property);
      depGraph[this._name][filter].dependencies.push({
        collection: dependency.collection,
        property: dependency.property
      });
      // register this filter as a dependency for the foreign filter or data property
      depGraph[dependency.collection][dependency.property].dependentNames.push(
        filter
      );
  ...
  findAllDependents(collection, propertyChanged) {
    if (!collection) collection = this._name;

    let graph = this._global.dependencyGraph;
    let dependents = graph[collection][filter].dependents;
  }
