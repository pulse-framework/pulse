...
module.exports = class Collection {
  ...
  initStorage() {
    ...
    this._storage.get = key => {
      return new Promise(async resolve => {
        let res;
        try {
          if (this._storageIsPromise) res = await this._global.storage.get(key);
          else res = this._global.storage.get(key);
  ...
  async initPersist(persist = []) {
    for (let property of persist) {
      ...
      let data = await this._storage.get(storageKey);

      // If data is null, set property instead
      if (data === null)
        return this._storage.set(storageKey, this._public[category][property]);

      // Set state within pulse from storage
      this._public[category][property] = data;
      if (this._public.hasOwnProperty(property)) this._public[property] = data;
