...
module.exports = class Collection {
  ...
  initProxy(obj = {}, rootProperty = false) {
    ...
    return new Proxy(objectWithCustomPrototype, {
      set: (target, key, value) => {
        ...
        //       this._name
        //     }". You can not mutate data from within a filter.`
        //   );
        //   return true;
        // }
        // allow proxy to be edited without intercepting
        if (
        ...
        // If the target is a deep reactive property
        ...
        } else if (Object.keys(this._public).includes(key)) {
          assert(
            `Cannot set data property "${key}" in collection "${
              this._name
  ...
  initDeepReactivity(target, key) {
    ...
    if (this.isWatchableObject(target[key])) {
      ...
      this._allowInternalChange = true;
      target[key] = this.initProxy(target[key], rootProperty);
      this._allowInternalChange = false;
