  ...
  parseModel(model) {
    Object.keys(model).forEach(property => {
      Object.keys(model[property]).forEach(config => {
        console.log("wefwfwWGWEGEWGEWGQEWGEWGwqer", property, config);
        if (config === "primaryKey") {
          this._primaryKey = model[property].primaryKey;
        } else if (config === "parent") {
          // if this is a valid collection
          // if (this._global.globalDataRefrence[model[property].parent]) {
          //   this._relations.push(model[property].parent);
          // } else {
          //   assert(`"${model[property].parent}" is not a valid collection.`);
          // }
        }
      });
    });
