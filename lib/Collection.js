...
module.exports = class Collection {
  constructor(
    ...
    console.log(`%c INIT ${this._name}`, 'color: #1bb9dc; padding:5px;');
  ...
  initStorage() {
    ...
    let asyncGet = key => {
      return new Promise(async resolve => {
        try {
          ...
          console.log(`%c READ ${key}`, 'color: red');
    ...
    this._storage.set = (key, value) => {
      console.log(`%c SET ${key}`, 'color: yellow');
    ...
    for (let property of persist) {
      ...
      if (this._storageIsPromise) {
        this._storage.get(storageKey).then(data => {
          ...
          //     storageKey,
          //     this._public[category][property]
          //   );
        ...
        console.log(`%c LOCAL STORAGE ${storageKey}`, 'color: red');
      ...
      // Log(`Data ${storageKey} loaded from LS`);
  ...
  updateSubscribers(key, data) {
    ...
    console.log(`%c Updating subscribers for ${key}`, 'color: #316EC6');
