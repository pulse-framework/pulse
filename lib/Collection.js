...
export default class Collection {
  constructor(
    ...
    this._mutableData = Object.create(null);
    ...
    Object.keys(model).forEach(property => {
      Object.keys(model[property]).forEach(config => {
        if (config === "primaryKey") {
          this._primaryKey = model[property].primaryKey;
  ...
  initData(data) {
    Object.keys(data).forEach(key => (this._mutableData[key] = undefined));
  ...
  initRoutes(routes) {
    ...
    for (let routeName of loop) {
      this._public.routes[routeName] = function(customParam) {
  ...
  prepareNamespace() {
    ...
    Object.keys(this._public).forEach(category => {
      // this[category] = this._public[category];
    ...
    return new Proxy(obj || {}, {
      set: (target, key, value) => {
        if (!this._global.initComplete) {
          target[key] = value;
        } else if (this._mutableData.hasOwnProperty(key)) {
  ...
  executeFilter(filter) {
    ...
    // update subscribers
    this.deliverFilterUpdate(data, filter);
  }

  filter(filter) {
    let from = filter.from;
    if (this._public.data.hasOwnProperty(from)) {
  ...
  buildDataFromIndex(index) {
    ...
    this.deliverGroupUpdate(data, index);
  }

  deliverGroupUpdate(data, group) {
    this._public.groups[group] = data;
    this.updateSubscribers(group, data);
  }

  deliverDataUpdate(data, dataName) {
    this._public.data[dataName] = data;
    this.updateSubscribers(dataName, data);
  }

  deliverFilterUpdate(data, filter) {
    this._public.filters[filter] = data;
    this.updateSubscribers(filter, data);
  }
