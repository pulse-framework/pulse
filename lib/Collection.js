...
module.exports = class Collection {
  ...
  initActions(actions) {
    ...
    for (let actionName of Object.keys(actions)) {
      ...
      this._public.actions[actionName] = function() {
        ...
        if (self._actionProcessingStack.length > 0)
          parentAction =
            self._actionProcessingStack[self._actionProcessingStack.length - 1]
              .actionIdentifier;

        // get context object
        const context = self.getContextObject();

        // replace default undo method with an action unique undo method
        context.undo = function(error) {
          return self.undo(actionName, actionIdentifier, error);
        };
        ...
        if (self._throttles.includes(actionName)) {
          self._onThrottleEnd[actionName] = actionPackage;
          return;
        }

        return self.executeAction(actionPackage);
      };
    }
  }
  ...
  throttle(amount) {
    ...
    setTimeout(() => {
      ...
      // call last reqested action on throttle end
      if (this._onThrottleEnd[actionToThrottle]) {
        setTimeout(() => {
          // invoke preserved action
          this.executeAction(this._onThrottleEnd[actionToThrottle]);
          // remove once complete
          delete this._onThrottleEnd[actionToThrottle];
        });
      }
    }, amount);
  }
