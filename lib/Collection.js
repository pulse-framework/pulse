...
module.exports = class Collection {
  ...
  initStorage() {
    ...
    if (
      ...
      // GET METHOD
      this._storage.get = key => this._global.storage.get(key);
  ...
  async initPersist(persist = []) {
    for (let property of persist) {
      ...
      let storageIsAsync = typeof this._storage.get.then == 'function';

      // if (storageIsAsync) data = await this._storage.get(storageKey);
      ...
      if (data) {
        if (type) {
          ...
          if (this._public.hasOwnProperty(property)) {
            if (!storageIsAsync) this._allowInternalChange = true;
            this._public[property] = data;
            if (!storageIsAsync) this._allowInternalChange = false;
          }
