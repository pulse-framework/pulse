...
module.exports = class Collection {
  ...
  buildGroupFromIndex(index) {
      ...
      for (let relationKey of Object.keys(this._groupRelations)) {
        ...
        if (data.hasOwnProperty(relationKey)) {
          ...
          this.emitToRoot('createForeignGroupRelation', {
            // foreign collection
            parentCollection: rel.fromCollectionName,
            parentGroup: data[relationKey],
    ...
    for (let collection of Object.keys(this._foreignGroupRelations)) {
      this.emitToRoot(
        'rebuildGroupsWithRelations',
        this._foreignGroupRelations[collection]
      );
    }
  ...
  // this function
  undo() {
    assert('undo requested, coming soon!');
  }
