  ...
  initRoutes(routes) {
    ...
    for (let routeName of loop) {
      ...
      this._public.routes[routeName] = function() {
        return routes[routeName](null, [_this._global.request].concat(Array.prototype.slice.call(arguments)));
      };
    }
  }
  ...
  initActions(actions) {
    for (let actionName of Object.keys(actions)) {
      ...
      this._public.actions[actionName] = function() {
        ...
        // run action
        let runAction = actions[actionName].apply(null, [context].concat(Array.prototype.slice.call(arguments)));
