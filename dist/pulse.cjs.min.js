...
(function (JobType) {
    ...
    JobType["FILTER_REGEN"] = "FILTER_REGEN";
...
var Runtime = /** @class */ (function () {
    ...
    Runtime.prototype.ingest = function (job) {
        if (this.ingestQueue.length > 0) {
            ...
            var alreadyInQueueAt = this.ingestQueue.findIndex(function (item) {
                return item.type === job.type &&
    ...
    Runtime.prototype.performJob = function (job) {
        ...
        if (job.dep && job.dep.dependents.size > 0) {
            ...
            job.dep.dependents.forEach(function (filter) {
                // get dep from public filter output
                _this.ingest({
                    type: JobType.FILTER_REGEN,
                    collection: filter.collection,
                    property: filter,
                    dep: _this.collections[filter.collection].public.getDep(filter.name)
                });
            });
        }
        this.finished();
    ...
    Runtime.prototype.performFilterOutput = function (job) {
        var filter = typeof job.property === 'string'
    ...
    Runtime.prototype.updateSubscribers = function (componentsToUpdate) {
        ...
        var _loop_1 = function (i) {
            ...
            var data = componentsToUpdate[componentID];
    ...
    Runtime.prototype.searchIndexes = function (collection, primaryKey) {
        var c = this.collections[collection];
        var foundIndexes = [];
        for (var i = 0; i < c.keys.indexes.length; i++) {
            var indexName = c.keys.indexes[i];
    ...
    Reactive.prototype.reactiveObject = function (object, rootProperty) {
        ...
        var _loop_1 = function (i) {
            ...
            if (Array.isArray(value)) {
                value = this_1.reactiveArray(value, key);
...
var Filter = /** @class */ (function () {
    function Filter(global, collection, name, filterFunction) {
        this.global = global;
        this.collection = collection;
        this.name = name;
    ...
    Filter.prototype.run = function () {
        ...
        var output = this.filterFunction(this.global.getContext(this.collection));
        if (output === undefined || output === null)
            output = false;
        this.global.runningFilter = false;
        return output;
    };
    Filter.prototype.addRelationToGroup = function (collectionName, groupName) {
        var parsed = JSON.stringify({
            collection: collectionName,
            group: groupName
        });
        if (!this.relatedToGroup.includes(parsed))
            this.relatedToGroup.push(parsed);
    };
    Filter.prototype.addRelationToInternalData = function (collectionName, primaryKey) {
        var parsed = JSON.stringify({
            collection: collectionName,
...
var Collection = /** @class */ (function () {
    ...
    Collection.prototype.prepareNamespace = function (root) {
        ...
        ['data', 'actions', 'filters', 'indexes', 'routes', 'watch'].forEach(function (type) {
        ...
        this.namespace = Object.assign(Object.create(__assign({}, this.methods)), __assign({ routes: {}, indexes: {}, actions: root.actions }, root.filters, root.data, this.normalizeGroups(root.groups)));
        return root;
    };
    ...
    Collection.prototype.initFilters = function (filters) {
        var _this = this;
        this.filters = {};
        objectLoop(filters, function (filterName, filterFunction) {
            _this.filters[filterName] = new Filter(_this.global, _this.name, filterName, filterFunction);
            _this.public.object[filterName] = [];
        }, this.keys.filters);
    };
    Collection.prototype.initRoutes = function (routes) {
        ...
        var routeWrapped = function (routeName) {
            return function () {
                ...
                return routes[routeName].apply(null, [self.global.request].concat(Array.prototype.slice.call(arguments)));
            };
        };
    ...
    Collection.prototype.buildGroupFromIndex = function (groupName) {
        ...
        for (var i = 0; i < index.length; i++) {
            var id = index[i];
            var data = Object.keys({}, this.internalData[id]);
            if (!data)
                continue;
    ...
    Collection.prototype.injectDataByRelation = function (data) {
        ...
        if (relations.length > 0)
            for (var i = 0; i < relations.length; i++) {
                var relationKey = relations[i];
                var rel = this.dataRelations[relationKey];
    ...
    Collection.prototype.collect = function (data, group, config) {
        ...
        for (var i = 0; i < data.length; i++) {
            var dataItem = data[i];
            var dataProcessed = this.processDataItem(dataItem, groups, config);
            if (dataProcessed)
                this.collectionSize++;
        }
    ...
    Collection.prototype.findById = function (id) {
        ...
        if (this.global.runningFilter) {
            var filter = this.global.runningFilter;
    ...
    Collection.prototype.increment = function (primaryKey, property, amount, decrement) {
        ...
        if (decrement)
            currentData[primaryKey][property] -= amount;
        else
            currentData[primaryKey][property] += amount;
        this.global.ingest({
            type: JobType.INTERNAL_DATA_MUTATION,
...
var Library = /** @class */ (function () {
    ...
    Library.prototype.runAllFilters = function () {
        var collectionKeys = Object.keys(this._private.collections);
        for (var i = 0; i < collectionKeys.length; i++) {
            var collection = this._private.collections[collectionKeys[i]];
            var filterKeys = collection.keys.filters;
            for (var i_1 = 0; i_1 < filterKeys.length; i_1++) {
                var filterName = filterKeys[i_1];
    ...
    Library.prototype.initCollections = function (root, request) {
        ...
        if (this._private.global.config.enableRequest !== false)
            this._private.collections['request'] = new Request(this._private.global, request);
    ...
    Library.prototype.getContext = function (collection) {
        ...
        return __assign({}, this._private.global.contextRef, c.methods, { data: c.public.object, indexes: c.indexes.object, groups: c.public.object, filters: c.public.object, routes: c.public.object.routes });
    };
    Library.prototype.install = function (Vue) {
        ...
        Vue.mixin({
            beforeCreate: function () {
                var _this = this;
                Object.keys(pulse.global.dataRef).forEach(function (collection) {
                    _this['$' + collection] = pulse.global.dataRef[collection];
                });
                if (pulse.utils)
    ...
    Library.prototype.mapData = function (properties, instance, _config, pulseAlias) {
        ...
        if (typeof properties === 'function') {
            ...
            normalizeMap(properties).forEach(function (_a) {
                ...
                returnData_1 = pulse._private.global.subs.subscribePropertiesToComponents(function () {
                    var _a;
