...
var Runtime = /** @class */ (function () {
    ...
    Runtime.prototype.updateSubscribers = function (componentsToUpdate) {
        ...
        for (var i = 0; i < componentKeys.length; i++) {
            ...
            var componentInstance = this.global.subs.componentStore[componentID];
            if (!componentInstance || !componentInstance.instance)
                return;
            var propertiesToUpdate = componentsToUpdate[componentID];
            Pulse.intergration.updateMethod(componentInstance.instance, propertiesToUpdate);
        }
    };
...
var SubController = /** @class */ (function () {
    ...
    SubController.prototype.analyseFunctionForReactiveProperties = function (func) {
        var deps = new Set();
        ...
        this.trackedDeps = new Set();
        this.trackAllDeps = false;
        var isMapData = !Array.isArray(evaluated) && typeof evaluated === 'object';
        return { isMapData: isMapData, deps: deps, evaluated: evaluated };
    };
    ...
    SubController.prototype.mapData = function (func, componentInstance) {
        var _this = this;
        // get container instance for component
        var componentContainer = this.get(componentInstance.__pulseUniqueIdentifier);
        var deps = new Set(), evaluated = {};
        if (typeof func === 'object') {
            // Pulse 1.0 compatiblity, should depricate soon!
            normalizeMap(func).forEach(function (_a) {
                var key = _a.key, val = _a.val;
                var moduleInstanceName = val.split('/')[0];
                var property = val.split('/')[1];
                var moduleInstance = _this.global.contextRef[moduleInstanceName];
                var res = _this.global.subs.analyseFunctionForReactiveProperties(function () {
                    var _a;
                    return _a = {}, _a[key] = moduleInstance[property], _a;
                });
                deps.add(res.dep);
            ...
            var res = this.global.subs.analyseFunctionForReactiveProperties(func);
            deps = res.deps;
            evaluated = res.evaluated;
        }
        // create subscription
        deps.forEach(function (dep) { return dep.subscribe(componentContainer); });
        return evaluated;
    };
...
function ReactWrapper(ReactComponent, depsFunc) {
    ...
    return /** @class */ (function (_super) {
        ...
        function class_1(props) {
            ...
            global.subs.registerComponent(_this, _this.config);
            return _this;
        }
        ...
        class_1.prototype.registerDeps = function (componentContainer) {
            var _a = global.subs.analyseFunctionForReactiveProperties(depsFunc), deps = _a.deps, isMapData = _a.isMapData, evaluated = _a.evaluated;
            this.isMapData = isMapData;
            deps.forEach(function (dep) { return dep.subscribe(componentContainer); });
            if (isMapData)
                this.mappedData = evaluated;
        };
        class_1.prototype.render = function () {
            var componentContainer = global.subs.get(this.__pulseUniqueIdentifier);
            var manualDepTracking = typeof depsFunc === 'function';
            if (manualDepTracking)
                this.registerDeps(componentContainer);
            else
                global.subs.trackingComponent = componentContainer;
            if (!global.subs.trackingComponent && !manualDepTracking)
                console.error('Pulse x React: React component not found!');
            var component;
            if (this.isMapData) {
                var props = this.props, old = global.config.mapDataUnderPropName;
                // support for 2.1 React props as pulse.mappedName but with ability to choose a custom name
                if (old)
                    props[old] = this.mappedData;
                else
                    props = __assign({}, props, this.mappedData);
        ...
        pulseConstructor.install = function (Vue) {
            ...
            Vue.mixin({
                beforeCreate: function () {
                    ...
                    // alias map
                    var mapData = global.subs.mapData.bind(pulse);
...
var use = (function (frameworkConstructor, PulseConstructor) {
    // React support
    ...
    else if (frameworkConstructor.hasOwnProperty('name') &&
...
var Pulse = /** @class */ (function () {
    ...
    Pulse.instance = null;
    Pulse.use = function (plugin) { return use(plugin, Pulse); };
    return Pulse;
}());
