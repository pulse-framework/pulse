...
function assert(func, funcName) {
  ...
  var warnings = {
    NO_PRIMARY_KEY: function() {
      return warn();
    },
    INVALID_PARAMETER: function() {
      return warn();
    },
    INDEX_NOT_FOUND: function() {
      return warn();
    },
    INTERNAL_DATA_NOT_FOUND: function() {
      return warn();
    },
    PROPERTY_NOT_A_NUMBER: function() {
      return warn();
    }
  };
...
var Runtime = /** @class */ (function() {
  ...
  Runtime.prototype.finished = function() {
    ...
    setTimeout(function() {
      if (_this.ingestQueue.length === 0) {
        if (!_this.updatingSubscribers) _this.compileComponentUpdates();
        _this.cleanup();
      } else {
        // loop more!
        _this.findNextJob();
  ...
  Runtime.prototype.performInternalDataUpdate = function(job) {
    ...
    if (!this.global.collecting) {
      ...
      var affectedIndexes = this.collections[
        job.collection
      ...
      affectedIndexes.forEach(function(index) {
        ...
        var modifiedGroup = _this.collections[
          job.collection
  ...
  Runtime.prototype.performInternalDataDeletion = function(job) {
    ...
    var indexesToUpdate = this.collections[
      job.collection
    ...
    for (var i = 0; i < indexesToUpdate.length; i++) {
      ...
      var newIndex = c.indexes.object[indexName].slice().filter(function(id) {
        return id !== job.property;
      });
      this.ingest({
  ...
  // ****************** Misc Handlers ****************** //
  Runtime.prototype.writeToPublicObject = function(
    collection,
    type,
    key,
    value
  ...
  Runtime.prototype.overwriteInternalData = function(
    collection,
    primaryKey,
    newData
  ) {
    ...
    var currentData = internalData[primaryKey]
      ? __assign({}, internalData[primaryKey])
      : false;
    if (currentData) {
...
(function(RelationTypes) {
  RelationTypes[(RelationTypes['COMPUTED_DEPENDS_ON_DATA'] = 0)] =
    'COMPUTED_DEPENDS_ON_DATA';
  RelationTypes[(RelationTypes['COMPUTED_DEPENDS_ON_GROUP'] = 1)] =
    'COMPUTED_DEPENDS_ON_GROUP';
  RelationTypes[(RelationTypes['DATA_DEPENDS_ON_DEP'] = 2)] =
    'DATA_DEPENDS_ON_DEP';
  RelationTypes[(RelationTypes['DATA_DEPENDS_ON_GROUP'] = 3)] =
    'DATA_DEPENDS_ON_GROUP';
  RelationTypes[(RelationTypes['DATA_DEPENDS_ON_DATA'] = 4)] =
...
var RelationController = /** @class */ (function() {
  ...
  RelationController.prototype.save = function(
    id,
    type,
    updateThis,
    whenThisChanges
  ...
  RelationController.prototype.saveTicketsByCleanupKey = function(
    cleanupKey,
    ticket
  ) {
  ...
  RelationController.prototype.relate = function(
    type,
    updateThis,
    whenThisChanges,
    collection // needed for data relations
  ) {
    ...
    if (collection) whenThisChanges = key(collection, whenThisChanges);
    // cleanup previous relations for this cleanup key
    var cleanupKey;
    // cleanup key is already set
    if (typeof updateThis === 'string') cleanupKey = updateThis;
...
var Reactive = /** @class */ (function() {
  ...
  Reactive.prototype.reactiveObject = function(object, rootProperty) {
    ...
    var _loop_1 = function(i) {
      ...
      if (Array.isArray(value));
        ...
        value = this_1.deepReactiveObject(
          value,
          rootProperty_1 || key,
          currentProperty
        );
  ...
  Reactive.prototype.deepReactiveObject = function(
    value,
    rootProperty,
    propertyOnObject
  ) {
...
var Collection = /** @class */ (function() {
  ...
  Collection.prototype.prepareNamespace = function(root) {
    ...
    collectionFunctions.map(function(func) {
      return (_this.methods[func] = _this[func].bind(_this));
    });
    ...
    this.namespace = Object.assign(
      Object.create(__assign({}, this.methods)),
      __assign(
        { routes: {}, indexes: {}, actions: root.actions },
        root.computed,
        root.data,
  ...
  Collection.prototype.initReactive = function(data, groups) {
    ...
    this.public = new Reactive(
      ...
      this.global,
      this.name,
      this.keys.data.concat(this.keys.indexes),
      'root'
    );
  };
  Collection.prototype.initPersist = function(persist) {
    ...
    var _loop_1 = function(i) {
      ...
      if (this_1.global.storage.isPromise) {
        ...
        if (data === undefined || data === null) return 'continue';
  ...
  Collection.prototype.initComputed = function(computed) {
    ...
    objectLoop(
      computed,
      function(computedName, computedFunction) {
        _this.computed[computedName] = new Computed(
          _this.global,
          _this.name,
          computedName,
          computedFunction
  ...
  Collection.prototype.injectDynamicRelatedData = function(primaryKey, data) {
    ...
    this.internalDataWithPopulate.forEach(function(property) {
      ...
      var populated = _this.model[property].populate(
        _this.global.getContext(),
        data
      );
  ...
  Collection.prototype.processDataItem = function(dataItem, groups, config) {
    ...
    this.searchIndexesForPrimaryKey(key).map(function(index) {
      return !affectedIndexes.includes(index) && affectedIndexes.push(index);
    });
  ...
  Collection.prototype.newGroup = function(groupName, indexValue) {
    if (this.indexes.object.hasOwnProperty(groupName))
      return assert(function(warn) {
        return warn.GROUP_ALREADY_EXISTS;
      });
    this.global.ingest({
      type: JobType.INDEX_UPDATE,
      collection: this.name,
  ...
  Collection.prototype.removeFromGroup = function(groupName, itemsToRemove) {
    ...
    var index = this.indexes.privateGet(groupName);
    var newIndex = index.filter(function(id) {
      return !itemsToRemove.includes(id);
    });
    this.global.ingest({
      type: JobType.INDEX_UPDATE,
      collection: this.name,
  ...
  Collection.prototype.update = function(primaryKey, newObject) {
    if (!this.internalData.hasOwnProperty(primaryKey))
      return assert(function(warn) {
        return warn.INTERNAL_DATA_NOT_FOUND;
      });
  ...
  Collection.prototype.increment = function(
    primaryKey,
    property,
    amount,
    decrement
    ...
    if (!validateNumber(amount, currentData[property]))
      return assert(function(warn) {
        return warn.PROPERTY_NOT_A_NUMBER;
      });
...
var SubController = /** @class */ (function() {
  ...
  SubController.prototype.subscribePropertiesToComponents = function(
    properties,
    componentUUID
  ) {
...
var Request = /** @class */ (function(_super) {
  ...
  function Request(global, requestConfig) {
    ...
    _this =
      _super.call(this, 'request', global, {
        groups: groups,
        data: data,
        persist: persist
      }) || this;
          ...
          case 8:
            ...
            if (this.responseIntercept) {
              ...
              this.responseIntercept(
                this.global.getContext('request'),
                response
              );
            }
    ...
    (function(_React$Component) {
      ...
      function _class(props) {
        ...
        _this = possibleConstructorReturn(
          this,
          getPrototypeOf(_class).call(this, props)
        );
        _this.state = objectSpread(
          ...
          pulse.mapData(
            ...
            {
              waitForMount:
                pulse._private.global.config.waitForMount === false
                  ? false
                  : true
            },
            pulse
...
var Library = /** @class */ (function() {
  function Library(root) {
    ...
    ['utils', 'services', 'staticData'].forEach(function(type) {
      if (root[type]) _this[type] = root[type];
    });
  ...
  Library.prototype.initCollections = function(root) {
    ...
    if (root.collections) {
      this._private.collectionKeys = Object.keys(root.collections).concat(
        this._private.collectionKeys
      );
    ...
    if (this._private.global.config.enableBase !== false) {
      ...
      this._private.collections['base'] = new Request$1(
        this._private.global,
        root
      );
    }
  ...
  Library.prototype.bindCollectionPublicData = function() {
    for (var i = 0; i < this._private.collectionKeys.length; i++) {
      var collection = this._private.collections[
        this._private.collectionKeys[i]
      ];
  ...
  Library.prototype.getContext = function(collection) {
    ...
    return __assign({}, this._private.global.contextRef, c.methods, {
  ...
  Library.prototype.install = function(Vue) {
    ...
    Vue.mixin({
      beforeCreate: function() {
        var _this = this;
        Object.keys(pulse._private.global.contextRef).forEach(function(
          collection
        ) {
          _this['$' + collection] =
            pulse._private.global.contextRef[collection];
        });
  ...
  Library.prototype.mapData = function(
    properties,
    instance,
    _config,
    pulseAlias
    ...
    var componentUUID = pulse._private.global.subs.registerComponent(
      instance,
      config
    );
